...
{
	// RZ Mar. 2019 - create a placeholder node if MathML is missing.
	if(in_node == null)
	{
		var result = document.createElementNS('http://www.w3.org/1998/Math/MathML', 'math');
		result.appendChild( document.createTextNode('[No MathML]') );
		return result
	}

	// OTHERWISE - create MathML copy.
...
trace_nodes_to_svg = function(trace_nodes, global_index, numChannels)
{
    ...
    var traces = trace_nodes.item(0).textContent
    var pattern = /\n+/g;
	pattern.compile(pattern);
	var point_text = traces.replace(pattern, "");
			...
			animate_traces = function(total_traces, trace_id, utc_ms, trace_speed, previous_offset)
			{
				...
				if(new_offset <= 0.0)
				{
					...
					if(trace_id == total_traces / 2)
					{
						...
						for(var k = 0; k < paths.length;k+=2)
						{
							paths.item(k).setAttribute("stroke-width", 2);
						}
						return;
					}
	...
	for(var k = 0; k < trace_nodes.length; k++)
	{
		...
		// RZ Mar 2019: Dirty - check first tuple in each stroke to get
		// the number of elements in each tuple.
		// First, split separately on commas; then count the number of elements
		// in the first 'token' (space-separated list of coordinates) as the
		// 'tupleLength.'
		pattern = /,/g;
		pattern.compile( pattern );
		var commas_in = point_text.split(pattern);

		pattern = /\s+/g;
		pattern.compile( pattern );
		tupleLength = commas_in[0].split( pattern ).length;

		for(var j = 0; j < point_strings.length; j+= tupleLength) //numChannels)
		{
			...
			if(j != 0)
			{
				// Some data has redundant first nodes,
				// so check for duplicate sequential nodes here
	...
	// Limit max height and width; constrain by one or the other.
	var Yscale = boxHeight / size_y; //2.0 / mean_distance;
	var Xscale = boxWidth / size_x;
...
build_attribute_table = function(annotation_nodes, filename, mathml)
{
	...
	td_attr_name.innerHTML = "<h3>File:</h3>";

	var td_attr_value = document.createElement("td");
	td_attr_value.setAttribute("class", "attribute_value");
	td_attr_value.innerHTML = "<h3>" + filename + "</h3>";

	var third_row = document.createElement("tr");
	third_row.appendChild(td_attr_name);
	third_row.appendChild(td_attr_value);
	result_table.appendChild(third_row);
...
on_xml_load = function(event)
{
	...
	for(var k = 0; k < file_list.length; k++)
	{
		...
		parseInkML = function(inkmlText,currentIndex)
		{
			...
			// RZ Mar 2019: if no mathml, do not build the stroke to mathml map.
			traceid_to_node_list[currentIndex] = {};
